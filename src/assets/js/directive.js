...
class VueTouch {
  ...
  start(e) {
    ...
    g.currentTarget=e.currentTarget;
    g.time = setTimeout(function () {
      if (!g.leaved && !g.moved) {
        g.touchType === "longtap" && g.callBack(e, g.firstTouchPosition,g.binding.value,g.currentTarget);
        g.longTouched = true;
      }
    }.bind(g), 700);
  }
  ...
  move(e) {
    this.moved = true;
    let {x, y} = this.getMultiCenter(e.changedTouches);
    let _disX = x - this.firstTouchPosition.x;
    let _disY = y - this.firstTouchPosition.y;
    let _dis = Math.sqrt(_disX * _disX + _disY * _disY);
    if(_dis<=2){
      this.moved = false;
    }
